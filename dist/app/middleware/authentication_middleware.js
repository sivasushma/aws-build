...
class AuthenticationMiddleware {
  ...
  authenticate(request, response, next) {
    _index.SessionsService.prototype.getCurrentUser(request.token).then(session => {
  ...
  authenticate_user(request, response, next) {
    _index.UsersService.prototype.getUserByEmail(request.body.email).then(user => {
      if (_index.PasswordService.compare_password(request.body.password, user.password)) {
        next();
      } else {
        next({
          code: 401,
