...
var _sessions_service = _interopRequireDefault(require("./sessions_service.js"));
  ...
  */
  creatNewUser(req, res) {
    return new Promise((resolve, reject) => {
      const parseReq = () => {
        return new Promise((resolve, reject) => {
          _utils.default.prototype.uploadUserProfilePics(req, res).then(() => resolve()).catch(error => reject(error));
        });
      };
      ...
      const checkDuplicateEntry = () => {
        return new Promise((resolve, reject) => {
          _index.User.findOne({
            tenant_id: _mongoose.default.Types.ObjectId(req.session.tenant_id),
        ...
        return new Promise((resolve, reject) => {
          ...
          _index.User.findOne({
            ...
            if (result) {
              ...
              const newUser = new _index.User();
              newUser.firstName = args.firstName;
              newUser.lastName = args.lastName;
              newUser.middleName = args.middleName;
              newUser.email = args.email;
              newUser.role = _mongoose.default.Types.ObjectId(args.role);
              newUser.tenant_id = _mongoose.default.Types.ObjectId(args.tenant_id);
              ...
              newUser.profile_pic = img; // if (args.building_id) newUser.building_id = mongoose.Types.ObjectId(args.building_id);

              if (args.building_id.length) {
                const buildings = JSON.parse(args.building_id).map(b => _mongoose.default.Types.ObjectId(b));
                newUser.building_id = buildings;
              } else {
                newUser.building_id = undefined;
              }

              newUser.reporting_manager = JSON.parse(args.reporting_manager).length ? JSON.parse(args.reporting_manager) : undefined; // if (args.stations_assigned) {
              ...
              if (args.stations_assigned.length) {
                const stations = JSON.parse(args.stations_assigned).map(s => _mongoose.default.Types.ObjectId(s));
              ...
              newUser.active = true;
              this.updateStationsWithUsers(newUser._id, newUser.stations_assigned, true, args.tenant_id).then(() => {
      ...
      parseReq().then(checkDuplicateEntry).then(createUserProfilePic).then(createUser).then(() => {
        resolve({
          status: 'Success',
          code: 200,
    ...
    return new Promise((resolve, reject) => {
      const rolesList = () => {
        return new Promise((resolve, reject) => {
          try {
            _index.Role.find({
              tenant_id: _mongoose.default.Types.ObjectId(args.tenant_id),
              status: true
            }, 'name _id').lean().exec().then(roleData => {
              resolve(roleData);
            }).catch(error => {
              reject({
                status: 'Fail',
      ...
      const getStationsData = () => {
        return new Promise((resolve, reject) => {
          try {
            ...
            _index.Floor.aggregate(asyncArr).exec().then(stationData => {
              resolve(stationData);
            }).catch(error => {
              reject({
                status: 'Fail',
        ...
        return new Promise((resolve, reject) => {
          ...
          if (args.station_id) {
            matchAggr['$match']['stations_assigned'] = {
              $elemMatch: {
                $eq: _mongoose.default.Types.ObjectId(args.station_id)
              }
            };
          }
          ...
          if (args.fromDate) {
            ...
            matchAggr['$match']['createdAt']['$gte'] = new Date(args.fromDate); //const status = DateValidator.prototype.validateFromToDates(args.fromDate, args.toDate, args.timezone);
          ...
          const groupByAggr = {
            $group: {
              _id: {
                _id: '$_id' //_id : '$stationDocs.station_id',
              ...
              stations_assigned: {
                $addToSet: {
                  $cond: [{
                    $gt: ['$stationDocs.station_id', null]
                  }, {
                    station_id: '$stationDocs.station_id',
              ...
              siteNames: {
                $addToSet: {
                  $cond: [{
                    $gt: ['$buildings.siteId', null]
                  }, {
                    siteId: '$buildings.siteId',
                    siteName: '$buildings.siteName'
                  }, '$$REMOVE']
                }
              },
              buildingNames: {
                $addToSet: {
                  $cond: [{
                    $gt: ['$buildings.builidingId', null]
                  }, {
                    builidingId: '$buildings.builidingId',
          ...
          _index.User.aggregate(asyncArr).exec().then(userlist => {
      ...
      Promise.all([rolesList(), getStationsData(), users()]).then(results => {
        resolve({
          rolesData: results[0],
    ...
    return new Promise((resolve, reject) => {
      const getUser = () => {
        return new Promise((resolve, reject) => {
          try {
            _index.User.findOne({
              _id: _mongoose.default.Types.ObjectId(req.params.user_id)
            }).then(data => {
              if (data == null) {
                reject({
                  status: 'Fail',
      ...
      const parseReq = () => {
        return new Promise((resolve, reject) => {
          _utils.default.prototype.uploadUserProfilePics(req, res).then(() => resolve()).catch(error => reject({
            status: 'Fail',
            code: 422,
            message: error.message
          }));
      ...
      const updateUser = img => {
        return new Promise((resolve, reject) => {
          try {
            ...
            if (args.role && args.role != '') user.role = _mongoose.default.Types.ObjectId(args.role);
            ...
            if (args.state && args.state != '') user.state = _mongoose.default.Types.ObjectId(args.state);
            if (args.country && args.country != '') user.country = _mongoose.default.Types.ObjectId(args.country);
            ...
            user.save((err, result) => {
              if (err) {
                ...
                delete userObj.updatedAt;
                delete userObj.password_history;
                this.updateStationsWithUsers(userObj._id, userObj.stations_assigned, userObj.active, userObj.tenant_id).then(() => resolve()).catch(error => {
                  console.log(error);
                  reject(error);
                });
              }
      ...
      getUser().then(parseReq).then(createUserProfilePic).then(updateUser).then(() => {
        resolve({
          status: 'Success',
          code: 200,
  ...
  */
  ...
  getCurrentUserEntities(args) {
    return new Promise((resolve, reject) => {
      const matchAggr = {
        $match: {
          _id: _mongoose.default.Types.ObjectId(args.user_id)
        }
      };
      const lookupAggr = {
        $lookup: {
      ...
      const groupByAggr = {
        $group: {
          ...
          site_name: {
            $addToSet: {
              $cond: [{
                $gt: ['$buildingDocs._id', null]
              }, {
                siteId: '$buildingDocs._id',
                siteName: '$buildingDocs.siteName'
              }, '$$REMOVE']
            }
          },
          building_name: {
            $addToSet: {
              $cond: [{
                $gt: ['$buildingDocs.buildingId', null]
              }, {
      ...
      _index.User.aggregate(asyncArr).exec().then(userData => {
        resolve(userData[0]);
      }).catch(error => reject({
        status: 'Fail',
        code: 422,
  ...
  async addUser(args) {
    return new Promise((resolve, reject) => {
      const userObj = new Promise((resolve, reject) => {
        try {
          if (args.id) {
            ...
            const match = {
              $match: {
                _id: _mongoose.default.Types.ObjectId(args.id)
              }
            };
            const unwindBuilding = {
              $unwind: {
            ...
            const groupByAggr = {
              $group: {
                ...
                country: {
                  $first: '$countryObj.name'
                },
                country_id: {
                  $first: '$countryObj._id'
                },
                state: {
                  $first: '$stateObj.name'
                },
            ...
            _index.User.aggregate(aggrStages).then(result => {
              if (result == null) {
                reject({
                  status: 'Fail',
                  code: 422,
      ...
      const countriesList = new Promise((resolve, reject) => {
        try {
          _index.Country.find({}, 'name _id').sort({
            'name': 1
          }).lean().exec().then(countryData => {
            resolve(countryData);
          });
        } catch (error) {
          reject({
      ...
      const buildingList = new Promise((resolve, reject) => {
        ...
        _index.Site.aggregate(aggrStages).then(data => {
          resolve(data);
        }).catch(error => reject({
          status: 'Fail',
          code: 422,
  ...
  async getStates(args) {
    return new Promise((resolve, reject) => {
      try {
        if (args.country) {
          _index.State.find({
            country: _mongoose.default.Types.ObjectId(args.country)
          }, 'name code _id').sort({
            name: 1
          }).exec().then(stateData => {
    ...
    return new Promise((resolve, reject) => {
      try {
        if (args.country && args.state) {
          _index.City.find({
            country: _mongoose.default.Types.ObjectId(args.country),
            state: _mongoose.default.Types.ObjectId(args.state)
          }, 'name _id zipcodes').sort({
            name: 1
          }).exec().then(citiesData => {
  ...
  */
  ...
  getCountry() {
    return new Promise((resolve, reject) => {
      ...
      _index.Country.aggregate([match, sort]).then(country => {
        resolve({
          status: 'success',
  ...
  */
  ...
  getSupervisorList(args) {
    return new Promise((resolve, reject) => {
      ...
      if (args.user_id) {
        matchAggr['$match']['_id'] = {
          $ne: _mongoose.default.Types.ObjectId(args.user_id)
        };
      }
      ...
      _index.User.aggregate(asyncArr).exec().then(rmList => {
        console.log(rmList);
        resolve(rmList);
      }).catch(error => reject(error));
    });
      ...
      const mapUsersToStations = () => {
        return new Promise((resolve, reject) => {
          _index.Floor.updateMany({
            ...
            $push: {
              'stations.$[elem].guard_id': _mongoose.default.Types.ObjectId(user_id)
            }
          }, {
            arrayFilters: [{
              'elem._id': {
      ...
      try {
        ...
        const matchAggr = {
          $match: {
            tenant_id: _mongoose.default.Types.ObjectId(args.tenant_id),
            active: true,
            building_id: _mongoose.default.Types.ObjectId(args.building_id)
          }
        };
        const unwindAggr = {
          $unwind: '$stations'
  ...
  */
  ...
  saveFamilyMember(req, res) {
    return new Promise((resolve, reject) => {
      ...
      const familiyMemberRoleProjection = {
        $project: {
          _id: 1
        }
      };
      ...
      const createUser = img => {
        return new Promise((resolve, reject) => {
          ...
          _index.Role.aggregate([getFamilyMemberRole, familiyMemberRoleProjection]).then(roleId => {
            ...
            _index.User.aggregate([checkFamilyMeber]).then(async result => {
              if (result.length) {
                reject({
                  code: 422,
                  name: 'unprocessable_entity',
                ...
                newUser.middleName = args.middleName;
                newUser.email = args.email;
                newUser.role = _mongoose.default.Types.ObjectId(roleId[0]._id); // DEfault family member role make it dynamic
      ...
      parseReq().then(checkExistingUser).then(createUserProfilePic).then(createUser).then(() => {
        resolve({
          status: 'Success',
          code: 200,
  ...
  */
  ...
  getFamilyMember(tenantId) {
    return new Promise((resolve, reject) => {
      ...
      _index.User.aggregate([userMatch, sort, month, project]).then(userArray => {
        if (userArray.length) {
          resolve({
            status: 'success',
  ...
  */
  ...
  updateFamilyMember(req, res) {
    return new Promise((resolve, reject) => {
      ...
      parseReq().then(createUserProfilePic).then(updateUser).then(data => {
        resolve(data);
      }).catch(error => reject(error));
    });
  ...
  */
  ...
  searchFamilyMember(filter, tenantId) {
    return new Promise((resolve, reject) => {
      ...
      _index.User.aggregate([match, sort, month, project]).then(data => {
        if (data.length) {
          resolve(data);
        } else {
          resolve([]);
    ...
    return new Promise((resolve, reject) => {
      const findUser = () => {
        return new Promise((resolve, reject) => {
          _index.User.findOne({
            ...
            password_history: 0,
            password: 0
          }).populate('tenant_id').lean().then(userData => {
            user = userData;
      ...
      const checkAccountExpiry = () => {
        return new Promise((resolve, reject) => {
          ...
          _index.User.aggregate(asyncArry).then(data => {
            if (data.length) {
      ...
      const roleDataFunc = () => {
        return new Promise((resolve, reject) => {
          ...
          _index.Role.aggregate(asyncArr).then(roleData => {
            role = roleData[0];

            if (role != undefined) {
              role.features = [];

              for (const i in role.access_list) {
                // eslint-disable-line
      ...
      const createSessionFunc = isExpired => {
        return new Promise((resolve, reject) => {
          ...
          _sessions_service.default.prototype.createUserSession(args).then(session => {
            const loginResp = _underscore.default.extend(user, {
              token: session.token,
              tenant_id: user.tenant_id._id,
              role: role.name,
              role_type: role.type,
      ...
      findUser().then(roleDataFunc).then(updateHomeUser).then(checkAccountExpiry).then(createSessionFunc).then(session => resolve(session)).catch(error => reject(error));
    });
  }
