export default class DateValidator {
  ...
  validateFromToDates(from, to, timezone) {
    let fromDate; let toDate; let currentDate;
    const now = new Date();
    ...
    if (fromDate && (currentDate - fromDate) < 0) {
      return {status: 'Fail', message: 'From date should be less than or equal to current Date' + (currentDate - fromDate)};
    }
    if (to && fromDate == undefined) {
      return {status: 'Fail', message: 'From date is required'};
    }
    ...
    if (toDate && (currentDate - toDate) < 0) {
      return {status: 'Fail', message: 'To date should be less than or equal to current date'};
    }
    if (fromDate && toDate && (toDate - fromDate) < 0) {
      return {status: 'Fail', message: 'From date should not be greater than To date'};
    }
    return {status: 'Success', fromDate, toDate};
  }
}
