...
export default class CountryStateService {
  ...
  async getCountries() {
    ...
    const countries = await axios(options);
  ...
  async getStatesByCountry(geonameId) {
    ...
    const statesData = await axios(options);
  ...
  async saveCountries(countriesData) {
      ...
      for (const i in countriesData) {
        if (countriesData[i]) {
          const countryObj =
            {
              updateOne: {
                filter: {name: countriesData[i].countryName},
                update: {name: countriesData[i].countryName,
    ...
    await Country.bulkWrite(countriesArr);
  }
  ...
  async saveStates() {
    ...
    const countriesData = await Country.aggregate(asyncArr);
    const asyncFunc = [];
    if (countriesData && countriesData.length>0) {
      for (const i in countriesData) {
        if (countriesData[i]) {
          ...
          asyncFunc.push(new Promise(async (resolve)=>{
            const statesList = await CountryStateService.prototype.getStatesByCountry(
                geonameId);
            const stateArr = [];
              ...
              for (const j in statesList) {
                if (statesList[j]) {
                  const stateObj =
                                {
                                  updateOne: {
                                    filter: {name: statesList[j].name, code: countryCode},
                                    update: {name: statesList[j].name,
            ...
            await State.bulkWrite(stateArr);
            resolve();
          }),
          );
        }
